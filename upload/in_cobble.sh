...
net=192.168.52.0  
#修改成dhcp计划分配的IP段
begin=192.168.52.250
end=192.168.52.253
echo "$ip    www.kbsonlong.com" >> /etc/hosts
...
echo 'default_password_crypted:  "$1$cobler$XJnisBweZJlhL651HxAM00"' >> /etc/cobbler/settings
cobbler get-loaders
service cobblerd restart
cobbler sync
...
option domain-name "kbsonlong.com";
...
service cobblerd restart
mkdir /opt/kbsonlong -p
mount /dev/cdrom /opt/kbsonlong 
cobbler import --name=rhel-6.6-x86_64 --path=/opt/kbsonlong